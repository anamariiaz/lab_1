...
submit.addEventListener("click", function(e){ 
    ...
    if (note_list.includes(note)){
        //if the inputed value is one of the 12 musical pitches in note_list at an index of 2 or greater, insert the note present at 2 indices prior into the html element with id "clarinet"
        if (note_list.indexOf(note)>1) {
            document.getElementById("clarinet").innerHTML=note_list[note_list.indexOf(note)-2]
        } 
        //if the inputed value is the musical pitch in note_list at an index of 0, insert 'A#' into the html element with id "clarinet"
        else if (note_list.indexOf(note)===0) {
            document.getElementById("clarinet").innerHTML=note_list[10]
        } 
        //if the inputed value is the 12 musical pitch in note_list at an index of 1, insert 'A' into the html element with id "clarinet"
        else if (note_list.indexOf(note)===1) {
            document.getElementById("clarinet").innerHTML=note_list[11]
        }
    } 
